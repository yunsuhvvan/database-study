...
-- TRUNC(A)         : 실수 A의 정수 절사값 ( 소수 아래 모두 잘라버린다.)
-- TRUNC(A,  1)     : 실수 A의 소수 1자리 초과.. 넘어가는 아래 절사값 
-- TRUNC(A,  2)     : 실수 A의 소수 2자리 초과.. 넘어가는 아래 절사값 
-- TRUNC(A, -1)     : 실수 A의 일의 자리 절사값 
-- TRUNC(A, -2)     : 실수 A의 십의 자리 절사값
SELECT TRUNC(1.99 ,1) , TRUNC(1.999 ,2) , TRUNC(999.999 , -1), TRUNC(999.999,-2)
  FROM DUAL;
...
-- 문제. 1.11 을 소수 1자리 올림 처리하여 1.2를  구하시오

SELECT CEIL(1.11*10) * 0.1
  FROM DUAL;

  
-- 문제. 111을 일의 자리 올림 처리하여 120을 구하시오.

SELECT CEIL(111 *0.1) / 0.1  -- 100 : 10의 2제곱
  FROM DUAL;
  
-- 문제. 1.111을 소수 2자리 올림 처리하여 1.12을 구하시오.
...
-- 문제. A를 N자리 올림 처리하시오
SELECT CEIL(123.1 * POWER(10 , 3)) / POWER(10, 2)
  FROM DUAL;
...
--     1) '날짜' +1 은 1일 후를 의미한다.
--     2) '날짜' -1 은 1일 전을 의미한다.
--     3) '날짜' +0.5 은 12시간 후를 의미한다.
--     4) '날짜' -0.5 은 12시간 전를 의미한다.

SELECT SYSDATE + 365 AS 내일
     , SYSDATE - 1 AS 어제
     , SYSDATE + 0.5 AS "12시간 후"
     , SYSDATE - 0.5 AS "12시간 전"
     , SYSTIMESTAMP +0.5 AS"12시간 후"
     , TO_CHAR(SYSDATE + 0.5 , 'YYYY/MM/DD HH:MI:SS') AS "12시간 후 "
     , TO_CHAR(SYSTIMESTAMP - 0.5 , 'YYYY/MM/DD HH:MI:SS') AS "12시간 전 "
...
--5. 경과한 개월 수 반환
--   MONTHS_BETWEEN(날짜1, 날짜2)  : 날짜1 - 날짜 2
SELECT name AS 사원명 
