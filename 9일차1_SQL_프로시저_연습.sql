...
-- order_id, customer_name, book_name, sales_price_ order_date 를 조회하시오.


SELECT o.order_id, c.customer_name, b.book_name, o.sales_price, o.order_date
  FROM orders o INNER JOIN customers c
    ON  c.customer_id = o.customer_id INNER JOIN books b
    ON  o.book_id = b.book_id
 WHERE order_date BETWEEN '21/09/04' AND '21/09/10';
 
SELECT o.order_id, customer_name, b.book_name, o.sales_price, o.order_date
  FROM orders o, customers c, books b
 WHERE c.customer_id = o.customer_id
   AND o.book_id = b.book_id
   AND order_date BETWEEN '21/09/04' AND '21/09/10';   
...
SELECT SUM(o.sales_price) , c.customer_name
...
SELECT a.customer_name, SUM(a.sales_price) AS 총구매액
  FROM (SELECT c.customer_name, o.sales_price, c.customer_id
          FROM customers c, orders o
         WHERE c.customer_id = o.customer_id
...
SELECT COUNT(*) AS 구매갯수, c.customer_name ,o.customer_id
  FROM customers c INNER JOIN orders o
    ON c.customer_id = o.customer_id
...
--4. '추신수'가 구매한 도서들의 출판사 수를 조회하시오

SELECT COUNT(*) , c.customer_name , o.book_id
  FROM customers c INNER JOIN orders o
    ON c.customer_id = o.customer_id
 GROUP BY c.customer_name , o.book_id
HAVING customer_name = '추신수';
...
SELECT c.customer_id ,c.customer_name , SUM(o.sales_price)
  FROM customers c INNER JOIN orders o
    ON c.customer_id = o.customer_id
 GROUP BY c.customer_name , c.customer_id;

-- teacher
SELECT c.customer_name ,c.customer_id, SUM(o.sales_price)
  FROM customers c, orders o
 WHERE c.customer_id = o.customer_id
 GROUP BY c.customer_name ,c.customer_id;


--6. 주문한 이력이 없는 고객의 customer_name 을 조회하시오
...
-- 1. BOOKS 테이블의 재고 (stock) 칼럼을 추가하고 모든 기존의 모든 책의 재고를 10개 세팅하시오.
ALTER TABLE BOOKS ADD STOCK NUMBER;
UPDATE BOOKS SET STOCK = 10;


--2. CUSTOMERS 테이블의 포인트(POINT) 칼럼을 추가하고 기존의 모든 고객의 포인트를 1000포인트로 세팅하시오
ALTER TABLE CUSTOMERS ADD POINT NUMBER;
UPDATE CUSTOMERS SET POINT = 1000;


-- ORDERS 테이블의 구매수량 (AMOUNT) 칼럼을 추가하고 기존의 모든 구매내역의 구매수량을 1로 세팅하시오.
...
-- 3.CUSTOMERS 테이블에 POINT를  UPDATE 한다. (총 주문액SALES_PRICE * AMOUNT)의 10% 적립
-- 4.BOOKS 테이블의 STOCK를 UPDATE 한다. (주문 수량만큼 감소시킨다.) 
