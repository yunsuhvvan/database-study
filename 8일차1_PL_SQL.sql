...
BEGIN
    my_name := '윤수환';
    my_age := 28;
    DBMS_OUTPUT.PUT_LINE(' 내이름은 ' || my_name|| '입니다');
...
BEGIN
    SELECT employee_id, first_name, last_name
     ...
     DBMS_OUTPUT.PUT_LINE('사원번호 : '|| v_employee_id || '입니다.');
     DBMS_OUTPUT.PUT_LINE('이름 : '|| v_first_name || '입니다.');
     DBMS_OUTPUT.PUT_LINE('성 : '|| v_last_name || '입니다.');
END;
...
DECLARE
    ...
    -- 레코드 타입 type_employee, 변수 선언 part
...
BEGIN
    ...
    ELSE
        result := '성인';
    END CASE;
    DBMS_OUTPUT.PUT_LINE('나이는 '|| age || '살이고, 저는' || result || '입니다.');
END;
...
-- 연습문제 . 1~100 사이 정수를 모두 더한 결과를 출력하시오(5050)
/*
DECLARE
    i NUMBER;
BEGIN
...
BEGIN
    ...
    WHILE i <= 100 LOOP
     ...
     i := i + 1;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('총합은 '||total||'입니다');
END;
...
BEGIN
    ...
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('전체 사원의 급여 총액은 ' || total || '원 입니다.');
END;
...
-- 12. 변수 선언 (테이블 타입)
-- 특정칼럼의 데이터들은 배열처럼 여러 데이터를 저장하는 구조를 사용한다.
...
DECLARE
    ...
    -- employee 테이블의 레코드 타입 (행타입)
    v_row employees%ROWTYPE;
BEGIN
    i := 0;
    
    FOR v_row IN (SELECT email FROM employees) LOOP
