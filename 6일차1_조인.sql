...
-- 4. 각 부서별 사원수와 평균 연봉을 department_name 과 함께 조회하시오.
--    사원수의 오름차순 정렬하시오
SELECT d.department_name , COUNT(*) AS 사원수 , AVG(e.salary) AS 평균연봉
  FROM departments d , employees e
 WHERE d.department_id = e.department_id
 GROUP BY d.department_name
 ORDER BY COUNT(*) ;   -- 한글도 된다
...
--5. city가 'S' 로 시작하는 지역에 근무하는 사원들의 employee_id, first_name , department_name, city 를 조회하시오
--    employees                departments           locations                    contries
--   department_id(FK)      department_id(pk)        country_id(fk)            country _id(pk)
--                          location_id (fk)          location_id(pk)        
...
-- 6. 모든 사원들의 employee_id , first_name, department_name, city ,country_name

SELECT  e.employee_id, e.first_name , d.department_name, l.city , c.country_name
  FROM  locations l,departments d,employees e,countries c
 WHERE  c.counrty_id = l.country_id
...
-- 7. manager 보다 먼저 입사한 사원의  employee_id , first_name , hire_date 와 manager의 hire_date
-- 사원테이블         : employees E
-- manager  테이블 : employees M
--  사원의 hire_date 가 manager 의 hire_date 보다 작은 사원을 조회하시오.

 SELECT e.employee_id , e.first_name , e.hire_date , m.hire_date
   FROM employees e, employees m
  WHERE e.employee_id = m.manager_id
    AND e.hire_date > m.hire_date;   --  잘못된것
    

 SELECT e.employee_id AS 사원번호 , e.first_name AS 사원명, e.hire_date AS 입사일 , m.hire_date AS 상사입사일
   FROM employees m, employees e
  WHERE  m.employee_id  = e.manager_id  -- 사원의 manager ID와 = manager의  employee_ID
 ...
 SELECT e.employee_id AS 사원번호 , e.first_name AS 사원명, e.hire_date AS 입사일 , m.hire_date AS 상사입사일
   FROM employees m INNER JOIN employees e
     ON  m.employee_id  = e.manager_id  -- 사원의 manager ID와 = manager의  employee_ID
  WHERE TO_DATE( m.hire_date) >TO_DATE(e.hire_date);
      
    
-- 8. 같은 부서 사원들 중에서 나보다 늦게 입사 하였으니 연봉을 더 많이 받는 사원이 있는 사원들의
--     department_id, first_name , salary , hire_date 와 높은 연봉을 받는 사원의 first_name , salary , hire_date 조회하시오
-- 나의 정보 : employees me
-- 너의 정보 : employees you
-- 조인 조건 : 같은 부서인가?

SELECT me.department_id AS 부서명 , me.first_name AS 사원명 , me.salary AS 급여, me.hire_date AS 입사일
       ,you.first_name AS 나보다연봉많은사원명 , you.salary AS 그사원의급여, you.hire_date AS 그사원의입사일
 ...
 -- 외부조인
 -- 한 테이블의 정보는 일치하면 조회, 다른 테이블의 정보는 일치하지 않아도 모두 조회
 -- 모든 정보를 조회할 테이블이 왼쪽에 있으면 왼쪽 조인, 오른쪽에 있으면 오른쪽 조인
 
   
-- 9. 모든 사원들의 employee_id, first_name, department_name 을 조회하시오
...
SELECT e.employee_id, e.first_name, NVL(d.department_name, '부서없음')
 ...
 WHERE d.department_id(+) = e.department_id --employees 테이블의 모든 정보는 조회 되도록 오른쪽 조인으로 설정
 ORDER BY e.employee_id;
   
SELECT e.employee_id, e.first_name, NVL(d.department_name, '부서없음')
  FROM departments d RIGHT JOIN employees e          -- 왼쪽 department , 오른쪽 employees
    ON d.department_id = e.department_id --employees 테이블의 모든 정보는 조회 되도록 오른쪽 조인으로 설정
 ORDER BY e.employee_id;
